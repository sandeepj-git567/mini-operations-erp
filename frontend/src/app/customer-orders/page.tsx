...
import { Plus, UserPlus, ShieldCheck, XCircle, AlertTriangle, ShoppingCart, RefreshCw } from 'lucide-react';
        ...
        <main className="flex-1 p-8 max-w-7xl mx-auto space-y-6">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 font-mono mt-0.5">
                    Sales order fulfillment & atomic PostgreSQL transaction stock locking
                  </p>
                </div>
              </div>
            </div>
            ...
            {canSales && (
              <div className="flex space-x-3">
                <button
                  ...
                  className="bg-slate-900 hover:bg-slate-800 text-slate-300 font-semibold px-4 py-2.5 rounded-xl border border-slate-700/80 transition-all flex items-center space-x-2 text-sm"
                >
                ...
                <button
                  ...
                  className="bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white font-semibold px-5 py-2.5 rounded-xl shadow-lg shadow-emerald-900/30 transition-all flex items-center space-x-2 text-sm border border-emerald-400/30"
                >
          ...
          {globalAlert && (
            <div className={`p-4 rounded-xl border font-mono text-xs flex items-center justify-between ${
              globalAlert.type === 'error' ? 'bg-rose-500/10 border-rose-500/30 text-rose-300' : 'bg-emerald-500/10 border-emerald-500/30 text-emerald-300'
            }`}>
              <span>{globalAlert.message}</span>
              <button onClick={() => setGlobalAlert(null)} className="text-slate-400 hover:text-white font-bold">✕</button>
            </div>
          )}
            ...
            <div className="p-4 bg-slate-900/60 border-b border-slate-800/80 flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="text-sm font-semibold text-white font-mono uppercase tracking-wider">
                  Active Sales Orders ({orders.length})
                </h3>
              </div>
              <button
                    ...
                    <tr>
                      <td colSpan={6} className="px-6 py-12 text-center text-slate-400 font-mono">
                        <div className="flex items-center justify-center space-x-2">
                          ...
                          <span>Loading customer orders stream...</span>
                        </div>
                      </td>
                    </tr>
                  ) : orders.length === 0 ? (
                    ...
                    orders.map((ord) => {
                      ...
                      return (
                        <tr key={ord.id} className="hover:bg-slate-800/40 transition-colors">
                          <td className="px-6 py-4 font-mono font-bold text-emerald-400">{ord.orderNumber}</td>
                          <td className="px-6 py-4">
                            <div className="font-semibold text-white">{ord.customer.name}</div>
                            <div className="text-xs text-slate-500 font-mono">{ord.customer.companyName}</div>
                          </td>
                          <td className="px-6 py-4">
                            {ord.items.map((item) => (
                              <div key={item.id} className="text-xs">
                                <span className="font-semibold text-slate-200">{item.item.name}</span>{' '}
                                <span className="text-slate-400 font-mono">({item.quantity} {item.item.unit} @ ₹{item.unitPrice})</span>
                              </div>
                            ))}
                          </td>
                          <td className="px-6 py-4 text-center font-bold text-cyan-300 font-mono">{totalQty} units</td>
                          ...
                          <td className="px-6 py-4 text-right space-x-2">
                            ...
                            {ord.status !== 'CANCELLED' && canSales && (
                              <button
                                ...
                                className="text-xs font-semibold px-2.5 py-1.5 rounded-lg bg-rose-500/10 text-rose-400 hover:bg-rose-500/20 border border-rose-500/30 transition-all inline-flex items-center space-x-1"
                              >
          ...
          <Modal isOpen={isCustomerModalOpen} onClose={() => setIsCustomerModalOpen(false)} title="Create New Customer">
            {custError && <div className="bg-rose-500/10 border border-rose-500/30 text-rose-300 p-3 rounded-xl text-xs font-mono mb-4">{custError}</div>}
            <form onSubmit={handleCreateCustomer} className="space-y-4">
              ...
              <div className="flex justify-end space-x-3 pt-4 border-t border-slate-800">
                <button type="button" onClick={() => setIsCustomerModalOpen(false)} className="px-4 py-2 text-sm font-semibold text-slate-400 hover:text-white">Cancel</button>
                <button type="submit" className="px-5 py-2 text-sm font-semibold text-white bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 rounded-xl shadow-lg shadow-cyan-900/30 border border-cyan-400/30">Save Customer</button>
              </div>
            </form>
          </Modal>
          ...
          <Modal isOpen={isOrderModalOpen} onClose={() => setIsOrderModalOpen(false)} title="Create New Customer Order">
            {orderError && <div className="bg-rose-500/10 border border-rose-500/30 text-rose-300 p-3 rounded-xl text-xs font-mono mb-4">{orderError}</div>}
            <form onSubmit={handleCreateOrder} className="space-y-4">
              <div>
                <label className="block text-xs font-mono text-slate-400 mb-1">Select Customer</label>
                <select
              ...
              <div>
                <label className="block text-xs font-mono text-slate-400 mb-1">Item Requested</label>
                <select
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-mono text-slate-400 mb-1">Unit Price (₹)</label>
                  <input
                    type="number"
                    min="1"
                    value={unitPrice}
              ...
              <div className="flex justify-end space-x-3 pt-4 border-t border-slate-800">
                <button type="button" onClick={() => setIsOrderModalOpen(false)} className="px-4 py-2 text-sm font-semibold text-slate-400 hover:text-white">Cancel</button>
                <button type="submit" className="px-5 py-2 text-sm font-semibold text-white bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 rounded-xl shadow-lg shadow-emerald-900/30 border border-emerald-400/30">Create Order Draft</button>
              </div>
            </form>
          </Modal>

          {/* Stock Reservation Modal */}
          <Modal isOpen={isReserveModalOpen} onClose={() => setIsReserveModalOpen(false)} title="Execute Stock Reservation">
            {reserveError && (
              <div className="bg-rose-500/10 border border-rose-500/30 text-rose-300 p-3 rounded-xl text-xs font-mono mb-4 flex items-start space-x-2">
            ...
            <form onSubmit={handleReserveStock} className="space-y-4">
              <div>
                <label className="block text-xs font-mono text-slate-400 mb-1">Warehouse Location for Stock Lock</label>
                <select
              ...
              <div className="p-3 bg-cyan-950/40 rounded-xl border border-cyan-500/30 text-xs font-mono text-cyan-300">
                <strong className="text-white">Transaction Safety:</strong> This action will execute an atomic PostgreSQL transaction with explicit row-locking (<code className="text-cyan-400">FOR UPDATE</code>). If stock is insufficient, transaction will rollback with HTTP 409 Conflict.
              </div>

              <div className="flex justify-end space-x-3 pt-4 border-t border-slate-800">
                <button type="button" onClick={() => setIsReserveModalOpen(false)} className="px-4 py-2 text-sm font-semibold text-slate-400 hover:text-white">Cancel</button>
                <button type="submit" className="px-5 py-2 text-sm font-semibold text-white bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 rounded-xl shadow-lg shadow-emerald-900/30 border border-emerald-400/30">Confirm & Lock Stock</button>
              </div>
            </form>
          </Modal>
        </main>
